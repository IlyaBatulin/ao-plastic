...
> УСТАРЕЛО. Не выполнять команды ниже. Актуальная подготовка и результаты
> проверки на 23.09.2026: [SELF-HOSTED-MIGRATION.md](SELF-HOSTED-MIGRATION.md).
> В проекте теперь используется Supabase Auth. Дамп только public, отключение
> GoTrue и откат простой сменой URL после новых записей небезопасны.
> Пароль Dashboard для переноса запрашивать не нужно.
